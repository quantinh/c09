...
WHERE a.id = d.architect_id
	...
	-- giá trị id của con phải trung với id của cha để lấy ra cái chung
	AND a.id IN 
		(
...
-- trình ở Vĩnh Long
-- Đầu tiên lấy ra tên các công nhân làm việc ở Cần Thơ
...
WHERE b.id = d.building_id
	...
	AND b.city = 'can tho'
	AND w.id NOT IN 
	-- Sau đó lấy ra tên các công nhân làm việc ở Vĩnh Long và so sánh với tên công nhân ở Cần Thơ theo id nếu trùng thì bỏ đi vế con bên trong trùng lấy của cha
	(
		SELECT w.id
		FROM building AS b, work AS d, worker AS w
		WHERE b.id = d.building_id
...
-- C2: Tìm mối liên hệ giữa các bảng
SELECT w.name, w.skill
